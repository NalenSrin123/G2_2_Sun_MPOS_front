  ...
  <div class="min-h-screen bg-gray-50 p-3 sm:p-4 lg:p-6">
    <div class="max-w-5xl mx-auto">
      ...
      <div class="flex items-center gap-2 mb-4 lg:mb-6 flex-wrap">
        <button
          @click="$router.push('/')"
          class="flex items-center gap-2 text-xs sm:text-sm text-gray-500 hover:text-emerald-700 transition-colors group"
        >
          <span class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center group-hover:border-emerald-300 group-hover:bg-emerald-50 transition-all">
        ...
        <span class="text-gray-300 text-xs">/</span>
        <span class="text-xs sm:text-sm text-emerald-700 font-medium truncate max-w-[120px] sm:max-w-none">{{ product.name }}</span>
      </div>
        ...
        <div class="flex flex-col lg:grid lg:grid-cols-5">
          ...
          <div class="lg:col-span-2 relative h-56 sm:h-72 lg:h-auto lg:min-h-[500px]">
            ...
            <div class="absolute top-3 left-3 sm:top-4 sm:left-4">
              <span class="inline-flex items-center gap-1.5 bg-emerald-700 text-white text-[10px] sm:text-xs font-bold tracking-wider uppercase px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg shadow">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                Chef's Pick
              </span>
            </div>
            <div class="absolute top-3 right-3 sm:top-4 sm:right-4">
              <span class="inline-flex items-center gap-1.5 bg-white/90 text-emerald-700 text-[10px] sm:text-xs font-semibold px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg">
          ...
          <div class="lg:col-span-3 flex flex-col p-4 sm:p-6 lg:p-7 gap-4 lg:gap-5">
            ...
            <div class="flex items-start justify-between">
              <div>
                ...
                <div class="flex items-baseline gap-1">
                  <span class="text-3xl sm:text-4xl font-bold text-emerald-700">${{ product.price }}</span>
            ...
            <div class="grid grid-cols-3 gap-2 sm:gap-3">
              <div v-for="n in nutrition" :key="n.label" class="bg-gray-50 rounded-xl p-2.5 sm:p-3 text-center border border-gray-100">
                ...
                <button
                  ...
                  :class="[
                    ...
                    selected === opt
                      ...
                      : 'bg-gray-50 border border-gray-200 text-gray-600 hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700'
                  ]"
                >{{ opt }}</button>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                class="w-full px-3 py-2.5 sm:px-4 sm:py-3 rounded-xl border border-gray-200 bg-gray-50 text-xs sm:text-sm text-gray-700 resize-none outline-none transition-colors focus:border-emerald-500 focus:bg-white placeholder-gray-300"
              />
            </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-100">
              <div class="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-xl p-1">
                <button @click="decreaseQty" class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-gray-500 hover:bg-white hover:text-emerald-700 transition-all text-base sm:text-lg font-light">−</button>
                <span class="text-sm font-bold text-gray-900 w-6 sm:w-8 text-center">{{ qty }}</span>
                <button @click="qty++" class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center text-gray-500 hover:bg-white hover:text-emerald-700 transition-all text-base sm:text-lg font-light">+</button>
              </div>
              <div class="flex-1">
              ...
              <button
                @click="addToCartAndGo"
                class="flex items-center gap-2 bg-emerald-700 hover:bg-emerald-800 text-white px-4 sm:px-6 lg:px-7 py-2.5 sm:py-3 rounded-xl font-semibold text-xs sm:text-sm transition-colors shadow-md shadow-emerald-700/20"
              >
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 sm:mt-5">
        <div v-for="info in infoCards" :key="info.title" class="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 flex items-center gap-3">
          <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0" :class="info.bg">
...
const infoCards = [
  { label: 'Preparation Time', value: '15–20 minutes', bg: 'bg-amber-50', color: 'text-amber-600', icon: '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>' },
  { label: 'Serving Size', value: '300g per portion', bg: 'bg-blue-50', color: 'text-blue-600', icon: '<path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/>' },
  { label: 'Origin', value: 'Kagoshima, Japan', bg: 'bg-emerald-50', color: 'text-emerald-700', icon: '<circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>' },
]

const product = ref({ name: '', subtitle: '', category: '', price: 0, description: '', image: '' })

<template>
  <div class="p-6 bg-gray-50 min-h-screen font-inter">
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
      <div class="bg-white rounded-xl p-4 flex items-center justify-between shadow-sm">
        ...
        <div class="w-11 h-11 rounded-xl bg-emerald-50 flex items-center justify-center">
          <svg class="w-5 h-5 text-emerald-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
        </div>
      </div>
      <div class="bg-white rounded-xl p-4 flex items-center justify-between shadow-sm">
        <div>
        ...
        <div class="w-11 h-11 rounded-xl bg-emerald-50 flex items-center justify-center">
          <svg class="w-5 h-5 text-emerald-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
        </div>
      </div>
      <div class="bg-white rounded-xl p-4 flex items-center justify-between shadow-sm">
        <div>
        ...
        <div class="w-11 h-11 rounded-xl bg-violet-50 flex items-center justify-center">
          <svg class="w-5 h-5 text-violet-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/></svg>
        </div>
      </div>
    ...
    <div class="flex items-center justify-between gap-3 mb-4 flex-wrap">
      ...
      <button class="flex items-center gap-1.5 text-sm text-gray-400 hover:text-emerald-800 transition-colors">
    ...
    <div class="hidden md:flex flex-col gap-2">
      ...
      >
        ...
        <div class="min-w-[110px] px-2">
          <p class="text-[10px] text-gray-400 uppercase tracking-wide mb-0.5">Category</p>
          <p class="text-sm text-gray-700">{{ item.cat }}</p>
        </div>

        <!-- Price -->
        <div class="min-w-[90px] px-2">
          <p class="text-[10px] text-gray-400 uppercase tracking-wide mb-0.5">Price</p>
          <p class="text-sm font-semibold text-gray-900">{{ item.price }}</p>
        </div>

        <!-- Status badge -->
        <div class="min-w-[120px] px-2">
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold" :class="stockBadge(item.qty)">
        ...
        <div class="min-w-[70px] px-2">
          <p class="text-[10px] text-gray-400 uppercase tracking-wide mb-0.5">Qty</p>
          <p class="text-sm font-semibold" :class="item.qty > 0 && item.qty < 10 ? 'text-amber-500' : 'text-gray-900'">{{ item.qty }}</p>
        </div>

        <!-- Actions -->
        <div class="flex items-center gap-2 pr-4 pl-2">
          <button @click="openEdit(item)" class="px-4 py-2 rounded-lg text-sm font-semibold border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors">Edit</button>
          <button @click="removeItem(item.id)" class="w-9 h-9 border border-gray-200 rounded-lg flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-gray-50 transition-colors">
    ...
    <!-- Footer -->
    <div class="flex items-center justify-between flex-wrap gap-3 mt-6 pt-5 border-t border-gray-100">
      <div class="flex gap-8 flex-wrap">
        <div>
          <p class="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Total Items</p>
          <p class="text-sm font-semibold text-gray-900 mt-1">{{ items.length }} Items</p>
        </div>
        <div>
          <p class="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Active Categories</p>
          <p class="text-sm font-semibold text-gray-900 mt-1">4 Groups</p>
        </div>
        <div>
          <p class="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Low Stock Alerts</p>
          <p class="text-sm font-semibold text-amber-500 mt-1">{{ alertCount }} Items</p>
        </div>
      </div>
  ...
  <Teleport to="body">
    <div v-if="showModal" class="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="closeModal">
      <div class="bg-white rounded-2xl w-full max-w-[440px] shadow-xl overflow-hidden" style="animation: pop .18s ease">
        <div class="flex items-center justify-between px-5 py-4 bg-emerald-50 border-b border-gray-100">
          <h2 class="text-sm font-bold text-gray-900">{{ editId ? 'Edit Item' : 'Add New Item' }}</h2>
          <button @click="closeModal" class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:bg-emerald-100 transition-colors">
        ...
        <div class="p-5 flex flex-col gap-4">
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Item Name</label>
            <input v-model="form.name" placeholder="e.g. Grilled Lamb Chops" class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-700 bg-gray-50 focus:bg-white transition-colors" />
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Description</label>
            <input v-model="form.desc" placeholder="Short description..." class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-700 bg-gray-50 focus:bg-white transition-colors" />
          </div>
          <div class="flex gap-3">
            <div class="flex flex-col gap-1.5 flex-1">
              <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Price</label>
              <input v-model="form.price" placeholder="$0.00" class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-700 bg-gray-50 focus:bg-white transition-colors w-full" />
            </div>
            <div class="flex flex-col gap-1.5 flex-1">
              <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Quantity</label>
              <input v-model.number="form.qty" type="number" min="0" class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-700 bg-gray-50 focus:bg-white transition-colors w-full" />
            </div>
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Category</label>
            <select v-model="form.cat" class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-700 bg-gray-50 focus:bg-white transition-colors">
              <option>Starters</option><option>Mains</option><option>Desserts</option><option>Drinks</option>
            </select>
          </div>
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-bold uppercase tracking-widest text-gray-400">Image URL</label>
            <input v-model="form.img" placeholder="https://..." class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-emerald-700 bg-gray-50 focus:bg-white transition-colors" />
          </div>
        </div>
        <div class="flex justify-end gap-2 px-5 py-4 border-t border-gray-100">
          <button @click="closeModal" class="px-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50 transition-colors">Cancel</button>
          <button @click="saveItem" class="px-4 py-2 bg-emerald-800 text-white text-sm font-semibold rounded-lg hover:bg-emerald-900 transition-colors">

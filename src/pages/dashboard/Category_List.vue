<template>
	<div class="min-h-screen bg-gray-100">
		<div class="container mx-auto px-3 sm:px-4 lg:px-6 py-6">
			<!-- Main Card -->
			<div class="w-full bg-white rounded-3xl shadow-sm p-4 md:p-6 lg:p-8">
				<!-- Header -->
				<div class="flex justify-between items-center mb-6">
					<div>
						<p class="text-xs md:text-sm text-gray-400 uppercase">
							Menu Management / Categories List
						</p>

						<h1
							class="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-800">
							Categories List
						</h1>
					</div>
					...
					<RouterLink
						to="/categories/new"
						class="w-12 h-12 md:w-14 md:h-14 rounded-full bg-emerald-700 text-white text-2xl md:text-3xl shadow-lg hover:bg-emerald-800 flex items-center justify-center">
						+
					</RouterLink>
				</div>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
					<div
						...
						:key="category.id"
						class="bg-white border rounded-2xl p-4 shadow-sm hover:shadow-lg transition duration-300">
						<div class="flex items-center gap-4">
							...
							<div class="flex-1">
								...
								<p class="text-sm text-gray-500">{{ category.items }} Items</p>
			...
			<RouterLink
				to="/categories/new"
				class="fixed bottom-4 right-4 md:bottom-8 md:right-8 w-14 h-14 md:w-16 md:h-16 rounded-full bg-emerald-700 text-white text-3xl shadow-xl hover:bg-emerald-800 flex items-center justify-center">
				+
			</RouterLink>
		</div>
	</div>
...
<script setup>
	...
	const categories = ref([
		...
		},
		{
			...
			active: true,
			image:
				"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSntBBf2Pz1BSUxdDuYc-udoXkyYNWG4R1lZNnH61W4D6g09KWLQgV7zrM&s",
		},
		{
			id: 6,
			name: "Asian Food",
